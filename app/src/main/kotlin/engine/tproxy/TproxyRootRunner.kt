...
internal class TproxyRootRunner(
    ...
    private fun TproxyStartConfig.buildBootScript(): String {
        return buildString {
            ...
            append(prepareBootRootCommand())
            append('\n')
            append(startDetachedCommand())
            append(waitForTproxyInboundCommand())
            append('\n')
    ...
    private fun TproxyStartConfig.prepareBootRootCommand(): String {
        return buildString {
            appendScript(
                """
                rm -f ${pidPath.shellQuote()} 2>/dev/null || true
    ...
    private fun TproxyStartConfig.waitForTproxyInboundCommand(): String {
        return """
            attempt=0
            while [ "${'$'}attempt" -lt $BootPortListenCheckAttempts ]; do
                netstat -an 2>/dev/null | grep 'LISTEN' | grep "[.:]$tproxyPort[[:space:]]" >/dev/null 2>&1 && exit 0
                attempt=${'$'}((attempt + 1))
                sleep 1
            done
