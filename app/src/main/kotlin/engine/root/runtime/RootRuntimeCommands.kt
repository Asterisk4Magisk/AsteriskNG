...
internal object RootShutdownOwnCommand {
    fun build(layout: RootRuntimeLayout): String {
        ...
        return shutdownWithoutEagerResponse + buildString {
            ...
            appendLine("  set +e")
            appendLine("  asteriskd_socket_probe=\"\$(awk '\$4 == \"00010000\" && \$6 == \"01\" && \$NF == \"@asteriskd.control\" { found = 1 } END { print found ? \"present\" : \"absent\" }' /proc/net/unix)\"")
            appendLine("  asteriskd_socket_probe_code=\"\$?\"")
            appendLine("  set -e")
            appendLine("  if [ \"\$asteriskd_socket_probe_code\" -ne 0 ]; then")
            appendLine("    printf '%s\\n' \"\$asteriskd_shutdown\"")
            appendLine("    exit \"\$asteriskd_socket_probe_code\"")
            appendLine("  fi")
            appendLine("  case \"\$asteriskd_socket_probe\" in")
            appendLine("    \"absent\") printf '%s\\n' \"\$asteriskd_shutdown\"; exit 0 ;;")
            appendLine("    \"present\") ;;")
            appendLine("    *) printf '%s\\n' \"\$asteriskd_shutdown\"; exit 1 ;;")
            appendLine("  esac")
